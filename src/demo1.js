...
var heroMapPos;//2D coordinates of hero map marker sprite in minimap
...
function update(){
    ...
    if (isWalkable())
    {
        ...
        heroMapSprite.x=heroMapPos.x;
        heroMapSprite.y=heroMapPos.y;
...
function createLevel(){
    ...
    heroMapPos=new Phaser.Point(heroMapSprite.x,heroMapSprite.y);
...
function drawHeroIso(){
    ...
    isoPt=cartesianToIsometric(heroMapPos);//find new isometric position for hero from 2D map position
    gameScene.renderXY(sorcerer,isoPt.x+borderOffset.x+heroWidth, isoPt.y+borderOffset.y-heroHeight, false);//draw to render texture
}
...
function isWalkable(){
    ...
    cornerTL.x=heroMapPos.x +  (heroSpeed * dX);
    cornerTL.y=heroMapPos.y +  (heroSpeed * dY);
